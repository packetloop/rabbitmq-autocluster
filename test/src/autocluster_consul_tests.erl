...
reg_body_service_only_test() ->
  ?assertEqual([{"ID", "test"}, {"Name", "test"}],
               autocluster_consul:registration_body("undefined", "test", "undefined", "undefined", undefined, undefined)).

reg_body_service_and_name_test() ->
  ?assertEqual([{"ID", "test"}, {"Name", "test"}, {"Tags", [foo]}],
               autocluster_consul:registration_body("undefined", "test", "foo", "undefined", undefined, undefined)).

reg_body_service_port_and_ttl_test() ->
  ?assertEqual([{"ID", "test"}, {"Name", "test"}, {"Port", 8501},
                {"Check", [{"Notes", ?CONSUL_CHECK_NOTES}, {"TTL", '45s'}]}],
               autocluster_consul:registration_body("undefined", "test", "undefined", "undefined", 8501, 45)).

reg_body_service_name_port_and_ttl_test() ->
  ?assertEqual([{"ID", "test"}, {"Name", "test"}, {"Port", 8501}, {"Tags", [foo]},
                {"Check", [{"Notes", ?CONSUL_CHECK_NOTES}, {"TTL", '45s'}]}],
               autocluster_consul:registration_body("undefined", "test", "foo", "undefined", 8501, 45)).

reg_body_service_with_prefix_test() ->
  ?assertEqual([{"ID", "01-test"}, {"Name", "test"}, {"Tags", [foo]}],
               autocluster_consul:registration_body("01", "test", "foo", "undefined", undefined, undefined)).
